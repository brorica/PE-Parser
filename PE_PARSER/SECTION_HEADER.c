...
{	
	...
	for (unsigned int i = 0; i < SectionNumber; i++)
	{
		...
		// FileAlignment ¹è¼ö ´ÜÀ§·Î ²÷±è
		realPointerToRawData = FileAlignment * ((PSECTION_HEADER + i)->PointerToRawData / FileAlignment);
		...
		printf("%08X\t%08X\t%-16s (%X)\n", (Offset += DWORDSIZE), (PSECTION_HEADER + i)->PointerToRawData, "PointerToRawData", realPointerToRawData);
